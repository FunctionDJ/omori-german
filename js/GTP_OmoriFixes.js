...
(function($) {
	...
	Scene_Boot = class extends Scene_Boot {
		...
		start() {
			...
			if(!!this.hasSteamwork()) {
				...
				throw new Error("Steam wurde nicht erkannt.")
			}
			if(!Utils.isOptionValid("test") && window.navigator.plugins.namedItem('Native Client') !== null) {
				throw new Error("Dieses Spiel funktioniert nicht im SDK Modus.")
			}
			if(Utils.isMac()) {
				const nw = require("nw.gui");
				const nw_window = nw.Window.get();
	...
	Game_Battler.prototype.addState = function(stateId) {
		...
		if(this.isEnemy()) {
			if(stateId === this.deathStateId() && this.name() === "SPACE EX-HUSBAND" && !this._immortalState) { // TODO funktioniert das???
	...
	DataManager = class extends DataManager {
		...
		static loadGameWithoutRescue(savefileId) {
			...
			if (this.isThisGameFile(savefileId)) {
				...
				if(!json) {throw new Error("Der Inhalt deiner Speicherdatei wurde verändert oder sie wurde korrupt");}
	...
	Yanfly.UCE.DataManager_loadGame = function(savefileId) {
		try {
			...
			else {alert("Problem beim Laden - " + e)};
			return false;
		}		
	}
