...
Scene_OmoriFile.prototype.onSelectInputOk = function() {
  ...
  if (index === 0) {
    ...
    if (StorageManager.exists(saveFileid)) {
      // Show Prompt Window
      this.showPromptWindow('Diese Datei überschreiben?');
    ...
    if (StorageManager.exists(saveFileid)) {
      // Show Prompt Window
      this.showPromptWindow('Diese Datei laden?');
      // Set Can select Flag to false
      this._canSelect = false;
    } else {
      // Play Buzzer Sound
...
Window_OmoriFileInformation.prototype.refresh = function() {
  ...
  this.contents.drawText('DATEI ' + id + ':', 10 + 30, -5, 100, this.contents.fontSize);
  // If Valid
  if (valid) {
    this.contents.drawText(info.chapter, 85 + 45, -5, this.contents.width, this.contents.fontSize);
    ...
    // Draw Total PlayTime
    this.contents.drawText('GESAMTE SPIELZEIT:', 118, 55, 200, 24);
    this.contents.drawText(info.playtime, 295 + 55, 55, 100, 24);
    // Draw Location
    this.contents.drawText('ORT:', 118, 80, 200, 24);
...
Window_OmoriFileCommand.prototype.makeCommandList = function () {
  this.addCommand("SPEICHERN", 'save', this._canSave);
  this.addCommand("LADEN", 'load', this._canLoad);
};
...
Window_OmoriFilePrompt.prototype.makeCommandList = function () {
  this.addCommand("JA", 'ok');
  this.addCommand("NEIN", 'cancel');
};
//=============================================================================
// * Refresh
//=============================================================================
